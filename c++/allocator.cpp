...
void test02(){
    allocator<string>alloc; //可以分配string的allocator的对象
    int n(5);
    ...
    std::cout << *(p+1) << std::endl; // 正确：使用string的输出运算符
    //std::cout << *q << std::endl; // 灾难：q指向未构造的内存
    std::cout << *(q-1) << std::endl; // 正确：q指向最后一个构造过的内存
    std::cout << p[0] << std::endl;
    std::cout << p[1] << std::endl;
    std::cout << p[2] << std::endl;

    while (q != p) {
        alloc.destroy(--q); //实现析构函数 释放我们真正构造的string
    }
